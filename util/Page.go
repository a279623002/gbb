...
func GetPagesInfo(tableName string, currentpage int, pagesize int, conditions string) (int, int, orm.RawSeter) {
	...
	o.Raw("SELECT count(*) FROM " + tableName + " " + conditions).QueryRow(&totalItem) //获取总条数
	...
	rs = o.Raw("select *  from  " + tableName + " " + conditions + " LIMIT " + con.Itoa((currentpage-1)*pagesize) + "," + con.Itoa(pagesize))
